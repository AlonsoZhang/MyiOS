...
@implementation ZWRequestManager


#pragma mark - POST_GET
// post
- (void)POST:(NSString *)urlString parameters:(id)parameters completion:(void (^)(ZWBaseResponse *))completion
{
    [self request:@"POST" URL:urlString parameters:parameters completion:completion];
}

//get
- (void)GET:(NSString *)urlString parameters:(id)parameters completion:(void (^)(ZWBaseResponse *))completion
{
...
- (void)request:(NSString *)method URL:(NSString *)urlString parameters:(id)parameters completion:(void (^)(ZWBaseResponse *response))completion
{
    ...
    if (self.currentNetworkStatus == AFNetworkReachabilityStatusNotReachable) {
        ZWBaseResponse *response = [ZWBaseResponse new];
...
#pragma mark - 加载本地数据
- (void)requestLocal:(NSString *)urlString completion:(void (^)(ZWBaseResponse *response))completion
{
...
#pragma mark - 处理数据
- (void)wrapperTask:(NSURLSessionDataTask *)task responseObject:(id)responseObject error:(NSError *)error completion:(void (^)(ZWBaseResponse *response))completion
{
    
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        
        ZWBaseResponse *response = [self convertTask:task responseObject:responseObject error:error];
...
#pragma mark - 包装返回的数据
- (ZWBaseResponse *)convertTask:(NSURLSessionDataTask *)task responseObject:(id)responseObject error:(NSError *)error
{
...
#pragma mark - 打印返回日志
- (void)LogResponse:(NSString *)urlString response:(ZWBaseResponse *)response
{
...
- (void)configSettings
{
    ...
    //自定义处理数据
    self.responseFormat = ^ZWBaseResponse *(ZWBaseResponse *response) {
        return response;
    };
...
static ZWRequestManager *_instance = nil;
