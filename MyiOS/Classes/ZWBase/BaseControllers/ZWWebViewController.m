...
- (void)viewDidLoad {
    ...
    ZWWeakSelf(self);
    [self.webView addObserverBlockForKeyPath:ZWKeyPath(weakself.webView, estimatedProgress) block:^(id  _Nonnull obj, id  _Nullable oldVal, id  _Nullable newVal) {
    ...
    if ([self webViewController:self webViewIsNeedAutoTitle:self.webView]) {
        ...
        [self.webView addObserverBlockForKeyPath:ZWKeyPath(self.webView, title) block:^(id  _Nonnull obj, id  _Nullable oldVal, id  _Nullable newVal) {
    ...
    [self.webView.scrollView addObserverBlockForKeyPath:ZWKeyPath(self.webView.scrollView, contentSize) block:^(id  _Nonnull obj, id  _Nullable oldVal, id  _Nullable newVal) {
...
- (UIView *)ZWNavigationBarLeftView:(ZWNavigationBar *)navigationBar
{
    ...
    self.closeBtn.ZW_x = leftView.ZW_width - self.closeBtn.ZW_width;
...
#pragma mark - ZWWebViewControllerDataSource
// 默认需要, 是否需要进度条
...
#pragma mark - ZWWebViewControllerDelegate
// 导航条左边的返回按钮的点击
    ...
    if(_webView == nil)
    {
        ...
        if ([self.parentViewController isKindOfClass:[UINavigationController class]]) {
            ...
            if ([self respondsToSelector:@selector(ZWNavigationHeight:)]) {
                ...
                webView.scrollView.contentInset = UIEdgeInsetsMake([self ZWNavigationHeight:nil], 0, 0, 0);
                // AppDelegate 进行全局设置
...
- (UIProgressView *)progressView
{
    if(_progressView == nil && [self.parentViewController isKindOfClass:[UINavigationController class]])
    {
        ...
        progressView.ZW_height = 1;
        
        progressView.ZW_width = kScreenWidth;
        
        progressView.ZW_y = self.ZW_navgationBar.ZW_height;
...
- (void)dealloc
{
    NSLog(@"ZWWebViewController -- dealloc");
